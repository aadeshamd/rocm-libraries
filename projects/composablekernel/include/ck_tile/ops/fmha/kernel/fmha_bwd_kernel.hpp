                         ...
                         const index_t* seqstart_ks = nullptr)
    {
        ...
        { // deterministic group mode (persistent)
            ...
            for(index_t b = 0; b < batch_size; ++b)
            {
                ...
                const index_t nc =
                    integer_divide_ceil(seqstart_ks[b + 1] - seqstart_ks[b], kN0);
                prefix_batch[b + 1] = prefix_batch[b] + nhead_q * nc * sq;
            }
            const index_t target_w =
                integer_divide_ceil(prefix_batch[batch_size], num_cus);
        ...
        if constexpr(kIsAvailable)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    const index_t w_hi = min(static_cast<index_t>((cu_id + 1) * target_w), total_w);
                    if(w_lo >= total_w)
                        ...
                        ++ibatch)
                    {
                        ...
                        const index_t head_start =
                            w_lo > pb ? static_cast<index_t>((w_lo - pb) / hw) : 0;

                        for(index_t head_idx = head_start; head_idx < kargs.nhead_q; ++head_idx)
                        {
                            const index_t w_head = pb + head_idx * hw;
                            if(w_head >= w_hi)
                                break; // remaining heads are past the interval

                            // chunk range: chunks satisfying w_lo <= w_head + c*sq < w_hi
                            const index_t wc_start =
                                w_lo > w_head ? static_cast<index_t>(w_lo - w_head) : 0;
                            ...
                            const index_t c_end = integer_divide_ceil(
                                min(hw, w_hi - w_head), sq);

                            for(index_t chunk_idx = c_start; chunk_idx < c_end; ++chunk_idx)
                                run_(kargs,
                                     dim3(chunk_idx, head_idx, ibatch),
                                     isplit,
                                     nsplits_b);
                        }
                    }
