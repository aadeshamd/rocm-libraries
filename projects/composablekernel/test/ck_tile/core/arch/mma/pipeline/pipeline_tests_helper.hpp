...
// ---------------------------------------------------------------------------
// Generates values as: ((row * minor_dim + col) % 7 + 1) * 0.25
// Small values avoid fp16 overflow, +1 avoids zeros, modular makes them non-uniform.
template <typename T>
...
// ---------------------------------------------------------------------------
// Register map helpers to scatter/gather between logical matrices and per-lane fragments.
// These operate on WaveTile-level arrays (e.g. InternalVec[FragsM][FragsK]).
// ---------------------------------------------------------------------------
                      ...
                      uint32_t waveSize)
{
    ...
    // Determine per-fragment external vector size
    // For dense: same as internal (a_vec_size)
    // For sparse: internal * kCompressionRatio
    constexpr index_t external_a_frag_vec_size = a_vec_size * kCompressionRatio;

    for(uint32_t lane = 0; lane < waveSize; ++lane)
    {
        // Get a byte pointer to the per-lane AVecType (which is a 2D array of fragment vecs)
        ...
        for(uint32_t bm = 0; bm < FragsM; ++bm)
        {
            ...
            {
                // Offset into the 2D array: [bm][bk] with each element having
                // external_a_frag_vec_size scalars
    ...
    for(uint32_t lane = 0; lane < waveSize; ++lane)
    {
        ...
        for(uint32_t bn = 0; bn < FragsN; ++bn)
        {
            for(uint32_t bk = 0; bk < FragsK; ++bk)
            {
                ...
                {
                    // B register map: major = N, minor = K
    ...
    for(uint32_t lane = 0; lane < waveSize; ++lane)
    {
        ...
        for(uint32_t bm = 0; bm < FragsM; ++bm)
        {
            for(uint32_t bn = 0; bn < FragsN; ++bn)
            {
                ...
                {
                    // C register map: major = M, minor = N
...
// ---------------------------------------------------------------------------
// Core test implementation (Pipeline type known at compile time)
// ---------------------------------------------------------------------------
    ...
    bool isSparse)
{
    ...
    // --- Scatter into per-lane fragments ---
    ...
    // --- Launch kernel via callback ---
    kernelLaunch(waveSize, d_a, d_b, d_c);
    HIP_CHECK_ERROR(hipDeviceSynchronize());

    // --- Copy back and extract ---
    ...
    for(uint32_t m = 0; m < M; ++m)
    {
        for(uint32_t n = 0; n < N; ++n)
        {
            float expected = static_cast<float>(C_expected[m * N + n]);
...
/// Run a full-matrix-verification pipeline test.
///
/// Queries the device compiler target at runtime via getDeviceCompilerTargetId(),
/// then dispatches to the correct Pipeline type for host-side register mapping.
///
    ...
    bool isSparse = false)
{
    ...
    // --- Query the actual compiler target from the device binary ---
    auto compilerTargetId = getDeviceCompilerTargetId();

    bool dispatched = dispatchCompilerTarget(compilerTargetId, [&](auto target) {
        ...
        run_pipeline_matrix_test_impl<Pipeline, AScalar, BScalar, CScalar>(
            M, N, K, waveSize, kernelLaunch, isSparse);
    });

    if(!dispatched)
    {
        GTEST_SKIP() << "Unrecognized compiler target id: "
                     << static_cast<uint32_t>(compilerTargetId);
    }
}
