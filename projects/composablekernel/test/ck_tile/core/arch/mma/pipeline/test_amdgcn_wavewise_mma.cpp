...
namespace {
const auto should_skip_dense = [](amdgcn_target_id currentArchId) {
    bool isSupportedMfma =
        (currentArchId >= amdgcn_target_id::GFX942) && (currentArchId <= amdgcn_target_id::GFX950);
    bool isSupportedWmma = (currentArchId >= amdgcn_target_id::GFX1100) &&
                           (currentArchId <= amdgcn_target_id::GFX12_GENERIC);
    return ((currentArchId == amdgcn_target_id::HOST) || !(isSupportedMfma || isSupportedWmma));
};
} // namespace

// PipelineFactory: maps a CompilerTarget type to a WaveWiseMmaPipeline type.
